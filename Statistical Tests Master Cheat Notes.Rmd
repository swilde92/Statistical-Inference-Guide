...
As a side note, bootstrapping can also be used in conjunction with a parametric test when we want to improve the accuracy of the power calculation (or sample size estimation) since this relies on the standard deviation of the statistic of interest. We can bootstrap this value to get a more reliable estimate for it to use in the power calculation. 

Another side note, if our hypothesis of interest is to **detect if there is no difference** between two groups, then permutation tests (some of the nonparametric tests listed above) are more appropriate. Remember the idea behind permutation tests is that we permute the data to mix the test and control group labels, calculate a new test statistic based on the permuted labels, and re-run this process until we have generated a distribution for the test statistic. We use this distrubtion to calculate a p-value with the observed test statistic. Since the nature of this type of test is randomly mixing the control and test groups to see if there really is a difference, it lends itself better to the scenario where we want to know if there is no difference between the groups. 
...
When assumptions aren't met for a comparison test, first try transofmring the data to obtain normality as long as it doesnt affect interpretability too bad (otherwise it can make giving actionable insights impossible at the end). If transformations dont work, then try a nonparametric test. The reason for trying a transformation first is nonparametric tests have a harder time detecting differences so they are often less insightful. Some common transformations are below:
